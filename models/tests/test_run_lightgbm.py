...
class TestGetFeatureColumns:
    ...
    def test_pc_features_detected(self, model_input_pc):
        cols = get_feature_columns(model_input_pc)
        pc_cols = [c for c in cols if c.startswith('PC')]
        assert len(pc_cols) == 551
    ...
    def test_location_included_by_default(self, model_input_pc):
    ...
    def test_location_year_included_by_default(self, model_input_pc):
        cols = get_feature_columns(model_input_pc)
        ly_cols = [c for c in cols if c.startswith('location_year_')]
        assert len(ly_cols) > 0

    def test_exclude_location_removes_pure_location(self, model_input_pc):
        """With include_location=False, location_AUS/PAK are absent."""
        cols = get_feature_columns(model_input_pc, include_location=False)
        pure_loc = [c for c in cols if c.startswith('location_')
                    and not c.startswith('location_year')]
        assert len(pure_loc) == 0

    def test_exclude_both_removes_all_location_features(self, model_input_pc):
        cols = get_feature_columns(model_input_pc, include_location=False,
                                   include_location_year=False)
        loc_cols = [c for c in cols if c.startswith('location')]
        assert len(loc_cols) == 0

    def test_genetic_only(self, model_input_pc):
        cols = get_feature_columns(model_input_pc, include_location=False,
                                   include_location_year=False)
        assert all(c.startswith('PC') for c in cols)

    def test_no_trait_or_metadata_columns(self, model_input_pc):
        cols = get_feature_columns(model_input_pc)
        assert 'sample.id' not in cols
        assert 'DTF_blue' not in cols
        assert 'location' not in cols
        assert 'location_year' not in cols

    def test_no_duplicate_columns(self, model_input_pc):
...
@pytest.fixture
def scaled_pc(model_input_pc):
    """model_input_pc with z-score scaling applied (as the real pipeline does)."""
    traits = [t for t in VALID_TRAITS if t in model_input_pc.columns]
    return apply_location_year_scaling(model_input_pc, traits)
...
class TestRunCV1:
    ...
    def test_returns_expected_keys(self, scaled_pc):
        out = run_cv1(scaled_pc, ['TGW_blue'],
    ...
    def test_predictions_have_expected_columns(self, scaled_pc):
        out = run_cv1(scaled_pc, ['TGW_blue'],
                      model_class=_QuickRegressor,
    ...
    def test_all_genotypes_with_data_predicted(self, scaled_pc):
        """Every genotype with non-null trait values should appear in predictions."""
        trait = 'TGW_blue'
        out = run_cv1(scaled_pc, [trait],
                      model_class=_QuickRegressor,
                      k_folds=5, n_iterations=1, min_genotypes=3)
        predicted_genos = set(out['predictions']['sample.id'].unique())
        genos_with_data = set(scaled_pc.loc[scaled_pc[trait].notna(), 'sample.id'].unique())
        assert predicted_genos == genos_with_data

    def test_seed_scheme_starts_at_1001(self, scaled_pc):
        out = run_cv1(scaled_pc, ['TGW_blue'],
                      model_class=_QuickRegressor,
                      k_folds=3, n_iterations=2, min_genotypes=3)
    ...
    def test_folds_differ_across_iterations(self, scaled_pc):
    ...
    def test_works_with_kinship_features(self, scaled_kinship):
        out = run_cv1(scaled_kinship, ['TGW_blue'],
                      model_class=_QuickRegressor,
                      k_folds=3, n_iterations=1, min_genotypes=3)
...
class TestRunCV2:
    ...
    def test_returns_expected_keys(self, scaled_pc):
        out = run_cv2(scaled_pc, ['TGW_blue'],
                      model_class=_QuickRegressor,
                      k_folds=3, n_iterations=1, min_genotypes=3)
        assert out['cv_scheme'] == 'CV2'
        assert 'results' in out

    def test_seed_scheme_starts_at_2001(self, scaled_pc):
        out = run_cv2(scaled_pc, ['TGW_blue'],
                      model_class=_QuickRegressor,
                      k_folds=3, n_iterations=2, min_genotypes=3)
        seeds = out['predictions']['seed'].unique()
    ...
    def test_observation_level_masking(self, scaled_pc):
        """CV2 masks at observation level — total predictions ≈ non-null obs."""
        out = run_cv2(scaled_pc, ['TGW_blue'],
                      model_class=_QuickRegressor,
                      k_folds=3, n_iterations=1, min_genotypes=3)
        n_obs = scaled_pc['TGW_blue'].notna().sum()
...
class TestRunCV0:
    ...
    def test_returns_expected_keys(self, scaled_pc):
        out = run_cv0(scaled_pc, ['TGW_blue'],
                      model_class=_QuickRegressor,
                      min_genotypes=3)
        assert out['cv_scheme'] == 'CV0'
        assert 'results' in out

    def test_deterministic_no_random_iterations(self, scaled_pc):
    ...
    def test_each_location_year_with_data_held_out(self, scaled_pc):
        """Each location-year that has non-null trait data should appear in predictions."""
        trait = 'TGW_blue'
        out = run_cv0(scaled_pc, [trait],
                      model_class=_QuickRegressor,
                      min_genotypes=3)
        ly_in_preds = set(out['predictions']['location_year'].unique())
        ly_with_data = set(
            scaled_pc.loc[scaled_pc[trait].notna(), 'location_year'].unique()
        )
...
class TestRunCrossLocation:
    ...
    def test_returns_expected_keys(self, scaled_pc):
        out = run_cross_location(scaled_pc, ['TGW_blue'],
                                 model_class=_QuickRegressor,
                                 min_genotypes=3)
        assert out['cv_scheme'] == 'CrossLocation'

    def test_uses_genetic_features_only(self, scaled_pc):
        """CrossLoc should use only PC/K_ features, no location encoding."""
        cols = get_feature_columns(scaled_pc, include_location=False,
                                   include_location_year=False)
        assert all(c.startswith('PC') or c.startswith('K_') for c in cols)
        assert not any(c.startswith('location_') for c in cols)
        assert not any(c.startswith('location_year_') for c in cols)

    def test_train_location_not_in_predictions(self, scaled_pc):
        out = run_cross_location(scaled_pc, ['TGW_blue'],
                                 model_class=_QuickRegressor,
                                 min_genotypes=3)
        preds = out['predictions']
...
class TestEvaluationCounts:
    ...
    def test_cv2_evaluation_count_matches_cv1(self, scaled_pc):
        ...
        trait = 'TGW_blue'
        k_folds, n_iter, min_g = 3, 2, 3
        cv1 = run_cv1(scaled_pc, [trait], model_class=_QuickRegressor,
                      k_folds=k_folds, n_iterations=n_iter, min_genotypes=min_g)
        cv2 = run_cv2(scaled_pc, [trait], model_class=_QuickRegressor,
                      k_folds=k_folds, n_iterations=n_iter, min_genotypes=min_g)
        assert len(cv2['results']) == len(cv1['results'])

    def test_cv2_all_location_years_in_every_fold(self, scaled_pc):
        """Each fold in CV2 should contain evaluations for all qualifying
        location-years — not just a random subset."""
        trait = 'TGW_blue'
        k_folds, min_g = 3, 3
        out = run_cv2(scaled_pc, [trait], model_class=_QuickRegressor,
    ...
    def test_cv0_evaluation_count(self, scaled_pc):
        """CV0: exactly one evaluation per qualifying location-year per trait."""
        trait = 'TGW_blue'
        min_g = 3
        out = run_cv0(scaled_pc, [trait], model_class=_QuickRegressor,
                      min_genotypes=min_g)
        n_ly = _n_qualifying_lys(scaled_pc, trait, min_g)
        assert len(out['results']) == n_ly

    def test_crossloc_evaluation_count(self, scaled_pc):
        """CrossLoc: for each training location, one evaluation per qualifying
        target location-year."""
        trait = 'TGW_blue'
        min_g = 3
        out = run_cross_location(scaled_pc, [trait],
                                 model_class=_QuickRegressor,
                                 min_genotypes=min_g)
        results = out['results']
        trait_data = scaled_pc[scaled_pc[trait].notna()]
...
class TestRunAllCVSchemes:
    ...
    def test_all_scheme_keys_present(self, scaled_pc):
        out = run_all_cv_schemes(scaled_pc, ['TGW_blue'],
                                 model_class=_QuickRegressor,
                                 k_folds=3, n_iterations=1,
                                 min_genotypes=3)
        assert {'cv1', 'cv2', 'cv0', 'cross_loc', 'all_results'} == set(out.keys())

    def test_all_results_combines_schemes(self, scaled_pc):
        out = run_all_cv_schemes(scaled_pc, ['TGW_blue'],
                                 model_class=_QuickRegressor,
                                 k_folds=3, n_iterations=1,
                                 min_genotypes=3)
