# One BLUE per accession per trait across all AUS years.
# Run from preprocessing/BLUEs/:  Rscript BLUEs_AUS.R

source("BLUEs_utils.R")

TRAITS <- c("DTF", "DTH", "PtHt", "PcleLng", "SdLen", "TGW", "SdW_z")
LOCATION <- "AUS"
OUTPUT_DIR <- "BLUEs_results/AUS"
MAX_ITERATIONS <- 20
QQ_CORRELATION_THRESHOLD <- 0.95

aus <- read.csv(
  "../../data/AUS_phenotypes_raw.csv",
  header = TRUE,
  na.strings = c("", "NA"),
  stringsAsFactors = FALSE
)
aus$year <- factor(aus$year)
aus$accession <- factor(aus$accession)

message(sprintf("[%s] rows: %d", LOCATION, nrow(aus)))
message(sprintf("[%s] rows per year:", LOCATION))
print(table(aus$year))

sequenced_accessions <- aus %>%
...
for (trait in TRAITS) {
  tryCatch({
    ...
    model_data <- aus %>%
      select(all_of(c("accession", "year", "SampleName", trait))) %>%
